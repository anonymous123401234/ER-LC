                        ...
                        # Command to run the Python script
                            for dec in "${debias_coeffiecient[@]}"; do
                                for bo in "${bucket_overlap[@]}"; do
                                    python CF_model_train_debias.py --gpu_id=3 --learning_rate=$lr --is_pairwise=False --ui_ratio=$ui_r --reason_ratio=$reason_ratio --neg_user_loss=$neg --ur_i_loss=$ur --ir_u_loss=$ir --u_ir_loss=$uir --reg_loss=$rgl --pop_bucket=$pop_bucket --debias_coeffiecient=$dec --bucket_overlap=$bo --pop_ratio=$pop_ratio --wandb_project=new_reason_CF
                                done
                            done
                        done
                    done
