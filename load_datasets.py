...
def traindata_get_reaons(user_context, item_context, seq_dataloader, logger, config, raw_dataset_itemid2model_itemid):
    model_item_id2raw_dataset_itemid = {}
    ...
    # pre_tokens = "<s>[INST] "
    # end_tokens = " [/INST]"
    pre_tokens, end_tokens = get_preend_tokens(config)
    ...
    for cur_data in seq_dataloader:
        ...
        for index, (user, item, history) in enumerate(zip(cur_data["user_id"], cur_data["item_id"], cur_data["item_id_list"])):
            user_samples[user.item()]["pos_item"].append(
                model_item_id2raw_dataset_itemid[item.item()])  # 添加到正样本列表
            user_samples[user.item()]["his_item"] = [model_item_id2raw_dataset_itemid[i]
    ...
    if prompt_type == "train_reason":
        ...
        if config["dataset"] == "ml-latest-small":
            if "7b" in config["model_name_or_path"]:
                ...
                cf_reasons = f"/reasons_cf_datasets/{cf_dataset_name}/user2reasons.json"
            else:
                raise NotImplementedError
        elif config["dataset"] == "Amazon_CDs_and_Vinyl_small":
            cf_dataset_name = config["cf_dataset_name"]
